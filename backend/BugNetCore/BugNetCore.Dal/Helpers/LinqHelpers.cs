namespace BugNetCore.Dal.Helpers
{
    public static class LinqHelpers
    {
        public static Expression<Func<T, bool>> BuildWherePredicate<T>(PropertyInfo propertyInfo, string filterQuery)
        {
            ...
            // For enum or nullable properties
            else if (propertyInfo.PropertyType.IsEnum)
            {
                var parameter = Expression.Parameter(typeof(T), "x");
                var property = Expression.Property(parameter, propertyInfo);

                var enumConstant = Expression.Constant(Enum.Parse(propertyInfo.PropertyType, filterQuery));
                var binaryExpression = Expression.Equal(property, enumConstant);
            ...
            else if (propertyInfo.PropertyType == typeof(Guid))
            {
                ...
                var guidConstant = Expression.Constant(Guid.Parse(filterQuery));
                var binaryExpression = Expression.Equal(property, guidConstant);
        ...
        public static string BuildWherePredicateForNestedProperty<T>(string filterOn, string filterQuery)
        {
            ...
            else if (deeplyNestedPropertyType == typeof(Guid))
            {
                constructedLambdaString = constructedLambdaString.Replace($"{segments[1]}", $"{segments[1]}.ToString().ToLower().Contains(\"{filterQuery}\")");
